...
flags.DEFINE_string('input_dir', '../tiny-imagenet-200', 'Input directory')#### was ''
flags.DEFINE_string('output_dir', '../tiny-imagenet-tfrecord', 'Output directory') ##### was ''
...
def read_validation_annotations(validation_dir):
  ...
  dir_results = read_tiny_imagenet_annotations(
    os.path.join(validation_dir, 'val_annotations.txt'),
    os.path.join(validation_dir, 'images'))
  random.shuffle(dir_results) ################# (out of 10,0000)
  dir_results = dir_results[0:3000]
  return dir_results
...
def read_training_annotations(training_dir):
  ...
  for sub_dir in sub_dirs:
    ...
    sub_dir_results = read_tiny_imagenet_annotations(
        ...
        one_label=sub_dir)
    random.shuffle(sub_dir_results)  ################# 20 * 200 (out of 500*200)
    sub_dir_results = sub_dir_results[:20] ##########
...
def read_test_annotations(test_dir):
  ...
  list = [(os.path.join(test_dir, 'images', f), None)
          for f in files if f.endswith('.JPEG')]
  random.shuffle(list) ################# (out of 10,0000)
  list = list[:3000]  ##########
  return list
